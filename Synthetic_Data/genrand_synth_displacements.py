...
# -*- coding: utf-8 -*-
"""
Created on Thu May 16 14:26:57 2019
...
def get_fault_parameters(size=(224,224)):
    ...
    zt            = random.choice(np.arange(0,20,1))         #depth to top of fault
    if zt <= 5:
        slip      = random.choice(np.arange(0.5,5,0.5))         #slip range every 1 meters
    else:
    ...
    Lp     = 1  #Constant
    Wp     = 1  #Constant
    FL     = random.choice(np.arange(10,50,1)) #Idealized fault length (km)
    FW     = random.choice(np.arange(5,30,1)) #Idealized fault width (km)
